package sunddylee.wehcat.com.service.impl;
...
//	public static int wechatCode = CommonConst.WECHAT_SERVICE_WKZF;
	 ...
	 * 
	 * @Autowired ActivityDataService activityDataService;
	 * 
	 * @Autowired RedpackService redpackService;
	 * 
	 * @Autowired WeixinQrCodeScanStatisticsService
	 * weixinQrCodeScanStatisticsService;
	 * 
	 * @Autowired WeixinMsgService weixinMsgService;
	 */
	...
	private static int wechatkeyCode = CommonConst.WECHAT_SERVICE_WKZF; // 服务号:
																		// 悟空找房
*/
	/**
	 * 处理微信发来的请求
			...
			// 事件推送
				...
				if (eventType.equals(MessageUtil.EVENT_TYPE_SUBSCRIBE)) {
					...
					if (!StringUtil.isNullOrEmpty(eventKey2)) {
						...
						if (arr.length >= 2) {
							...
							weixinQrCodeScanStatisticsService
									.handleWeiXinQrCodeScan(qrScan);
							respContent = processMsg(content, request,
									textMessage);
						}
					}
					// 扫描
				...
				// 自定义菜单点击事件
					...
					if (content.equals("V1002_4")) {
						...
						respContent = MessageUtil.ResNewsMessage(fromUserName,
								toUserName, articleList);
					} else {
						respContent = processMsg(content, request, textMessage);
					}
				...
				logger.info("this weixin scan info is:MsgType=" + msgType
						+ ",eventType=" + eventType + ",ToUserName="
	...
	/*// 根据消息绑定信息
	public String processMsg(String msg_content, HttpServletRequest res,
			TextMessageResponse textMessage) throws Exception {
		StringBuffer buffer = new StringBuffer();

		// 处理逻辑:"悟空找房超级棒(发放红包)"
		// if (StringUtil.equal("李壮测红包", msg_content)) {
		// return redpackService.signUpWeixinUserByRedpack(msg_content, res,
		// textMessage);
		// }
		 ...
		 * 
		 * textMessage.setContent(buffer.toString()); return
		 * PullMessageUtil.textMessageToXml(textMessage); }else if(result ==
		 * -2){ buffer.append("您输入的格式不对,格式为: 公司+员工+电话号码。").append("\n");
		 ...
		 * textMessage.setContent(buffer.toString()); return
		 * PullMessageUtil.textMessageToXml(textMessage); }else if(result ==
		 * -4){ buffer.append("抱歉，更新失败，请确认您是否已报名。");
		 * textMessage.setContent(buffer.toString()); return
		 * PullMessageUtil.textMessageToXml(textMessage); } }
	...
	*//**
	...
	public JSSignResponse getJsSign(String requestUrl) throws Exception {
		String jsrJson = FasterJsonTool.writeValueAsString(rmiWechatService
				.getJsSDKConfigInfo(wechatCode, requestUrl).getData());
	...
	*//**
	 ...
	 * @return
	 *//*
	public ResponseUtil addWeixinImageMsg(Map<String, String> requestMap)
			throws Exception {
		BaseMessageRequest baseMessageRequest = fetchBaseMessageRequest(requestMap);
		...
		// 从微信服务器 下载图片
		byte[] image = WechatUtil.mediaFetch(rmiWechatService.getAccessToken()
				.getData(), mediaId);
	...
	*//**
	 ...
	 * @return
	 *//*
	public BaseMessageRequest fetchBaseMessageRequest(
	...
	public Oauth2WeixinUserRequest getOauth2WeiXinUser(
			Oauth2AccessRequest oauth2AccessRequest) throws Exception {
		Response<String> oauth2WeixinUserRequestResponse = rmiWechatService
				.getOauth2WeiXinUser(wechatkeyCode, oauth2AccessRequest);
		String strJson = FasterJsonTool
				.writeValueAsString(oauth2WeixinUserRequestResponse.getData());
		logger.info("getOauth2WeiXinUser strJson: " + strJson);
		return FasterJsonTool.readValue(strJson, Oauth2WeixinUserRequest.class);
	}

	public Oauth2AccessRequest getOauth2AccessToken(String code)
			throws Exception {
		Response<String> oauth2AccessTokenResponse = rmiWechatService
				.getOauth2AccessToken(wechatCode, code);
		String jsonData = FasterJsonTool
				.writeValueAsString(oauth2AccessTokenResponse.getData());
	...
	public String getOauth2Code(String requestUrl) throws Exception {
		Response<String> oauth2Code = rmiWechatService.getOauth2Code(
				wechatCode, requestUrl, "snsapi_base");// 调用weixinSOA授权访问
														// 不弹出授权框模式
	...
	@Override
	public JSSignResponse getJsSign(String requestUrl, Integer wechatCode)
			throws Exception {
		if (null == wechatCode) {
